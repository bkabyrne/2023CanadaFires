...
def calc_forest_fires(year,prior,post,areain,Forest_maskin):
    # =============
    prior_vec = np.zeros(np.size(prior[:,0,0]))
    post_vec = np.zeros(np.size(prior[:,0,0]))
    # =============
    for i in range(np.size(prior[:,0,0])):
        #
        #print(i)
        #
        prior_temp = prior[i,:,:] * areain
        prior_temp[np.where(Forest_maskin==0)] = np.nan
        prior_vec[i] = np.nansum(prior_temp)
        #
        post_temp = post[i,:,:] * areain
        post_temp[np.where(Forest_maskin==0)] = np.nan
        post_vec[i] = np.nansum(post_temp)
    # =============
    if np.logical_and(year % 4 == 0, year % 100 != 0):
        print('leap year!')
        post_MaySep = np.nansum(post_vec[121-1+1:274-1+1])
        prior_MaySep = np.nansum(prior_vec[121-1+1:274-1+1])
    else:
        post_MaySep = np.nansum(post_vec[121-1:274-1])
        prior_MaySep = np.nansum(prior_vec[121-1:274-1])
    # =============
    return prior_MaySep, post_MaySep


file_in = './data_for_figures/TROPOMI_GFED_COinv_2x25_2023_fire_3day_MonthCarlo.nc'
f = Dataset(file_in,'r')
CO_prior = f.variables['CO_prior'][:] # gC/m2/day
CO_post = f.variables['CO_post'][:] # gC/m2/day
CO2_prior = f.variables['CO2_prior'][:] # gC/m2/day
CO2_post = f.variables['CO2_post'][:] # gC/m2/day
area_2x25 = f.variables['grid_area'][:] # m2
f.close()

CO2_post_MaySep = np.zeros(40)
CO_post_MaySep = np.zeros(40)
for i in range(40):
    CO2_prior_MaySep, CO2_post_MaySep[i] = calc_forest_fires(2023,CO2_prior[i,:,:,:],CO2_post[i,:,:,:],area_2x25,Forest_mask_2x25)
    CO_prior_MaySep, CO_post_MaySep[i] = calc_forest_fires(2023,CO_prior[i,:,:,:],CO_post[i,:,:,:],area_2x25,Forest_mask_2x25)

GFED_ensemble = ( CO2_post_MaySep+CO_post_MaySep ) * 1e-12
print(np.std(GFED_ensemble))
print(np.mean(GFED_ensemble))
print('----')

file_in = './data_for_figures/TROPOMI_QFED_COinv_2x25_2023_fire_3day_MonthCarlo.nc'
f = Dataset(file_in,'r')
CO_prior = f.variables['CO_prior'][:] # gC/m2/day
CO_post = f.variables['CO_post'][:] # gC/m2/day
CO2_prior = f.variables['CO2_prior'][:] # gC/m2/day
...
QFED_ensemble = ( CO2_post_MaySep+CO_post_MaySep ) * 1e-12
print(np.std(QFED_ensemble))
print(np.mean(QFED_ensemble))
print('----')    

file_in = './data_for_figures/TROPOMI_GFAS_COinv_2x25_2023_fire_3day_MonthCarlo.nc'
f = Dataset(file_in,'r')
CO_prior = f.variables['CO_prior'][:] # gC/m2/day
CO_post = f.variables['CO_post'][:] # gC/m2/day
CO2_prior = f.variables['CO2_prior'][:] # gC/m2/day
...
GFAS_ensemble = ( CO2_post_MaySep+CO_post_MaySep ) * 1e-12
print(np.std(GFAS_ensemble))
print(np.mean(GFAS_ensemble))
print('----')    
...
for ii in range(12):
    ...
    f.close()
    #
    CO2_prior_MaySep, CO2_post_MaySep = calc_forest_fires(yyyy,CO2_prior,CO2_post,area_2x25,Forest_mask_2x25)
    CO_prior_MaySep, CO_post_MaySep = calc_forest_fires(yyyy,CO_prior,CO_post,area_2x25,Forest_mask_2x25)
    #
    prior_MaySep_MOPITT_vec[ii] = CO2_prior_MaySep+CO_prior_MaySep
    post_MaySep_MOPITT_vec[ii] = CO2_post_MaySep+CO_post_MaySep
# ----------------------------------------------------------------------------------------


# ----------------------------------------------------------------------------------------
# Calculate GFED posterior May-Sep Fire emissions for each year for the TROPOMI inversions
prior_MaySep_TROPOMI_vec = np.zeros(5)
...
for ii in range(5):
    ...
    #
    #
    prior_MaySep_TROPOMI_vec[ii] = CO2_prior_MaySep+CO_prior_MaySep
    post_MaySep_TROPOMI_vec[ii] = CO2_post_MaySep+CO_post_MaySep
# ----------------------------------------------------------------------------------------
...
CO_prior_MaySep, CO_post_MaySep = calc_forest_fires(yyyy,CO_prior,CO_post,area_2x25,Forest_mask_2x25)
#
prior_MaySep_TROPOMI_QFED_vec = CO2_prior_MaySep+CO_prior_MaySep
post_MaySep_TROPOMI_QFED_vec = CO2_post_MaySep+CO_post_MaySep
# ----------------------------------------------------------------------------------------
...
CO_prior_MaySep, CO_post_MaySep = calc_forest_fires(yyyy,CO_prior,CO_post,area_2x25,Forest_mask_2x25)
#
prior_MaySep_TROPOMI_GFAS_vec = CO2_prior_MaySep+CO_prior_MaySep
post_MaySep_TROPOMI_GFAS_vec = CO2_post_MaySep+CO_post_MaySep
# ----------------------------------------------------------------------------------------


# ----------------------------------------------------------------------------------------
# Calculate GFED prior May-Sep Fire emissions for each year for the TROPOMI inversions
...
prior_GFED_MaySep_vec = np.zeros(21)
for ii in range(21):
    #
    prior_temp =GFED_total_Flux_per_area[ii,:,:] * area
    prior_temp[np.where(Forest_mask==0)] = np.nan
    prior_GFED_MaySep_vec[ii] = np.nansum(prior_temp)
    #
# ----------------------------------------------------------------------------------------


# ----------------------------------------------------------------------------------------
# Calculate QFED prior May-Sep Fire emissions for each year for the TROPOMI inversions
nc_out = './data_for_figures/QFED_monthly_MERRA2_regrid_MaytoSep.nc'
f = Dataset(nc_out,'r')
QFED_CO_Flux_per_area = f.variables['CO_Flux'][:] # gC/m2
QFED_CO2_Flux_per_area = f.variables['CO2_Flux'][:] # gC/m2
f.close()
QFED_total_Flux_per_area = QFED_CO2_Flux_per_area + QFED_CO_Flux_per_area
prior_QFED_MaySep_vec = np.zeros(21)
for ii in range(21):
    #
    prior_temp =QFED_total_Flux_per_area[ii,:,:] * area
    prior_temp[np.where(Forest_mask==0)] = np.nan
    prior_QFED_MaySep_vec[ii] = np.nansum(prior_temp)
    #
# ----------------------------------------------------------------------------------------


# ----------------------------------------------------------------------------------------
# Calculate GFAS prior May-Sep Fire emissions for each year for the TROPOMI inversions
nc_out = './data_for_figures/GFAS_monthly_MERRA2_regrid_MaytoSep.nc'
f = Dataset(nc_out,'r')
GFAS_CO_Flux_per_area = f.variables['CO_Flux'][:] # gC/m2
GFAS_CO2_Flux_per_area = f.variables['CO2_Flux'][:] # gC/m2
f.close()
GFAS_total_Flux_per_area = GFAS_CO2_Flux_per_area + GFAS_CO_Flux_per_area
prior_GFAS_MaySep_vec = np.zeros(21)
for ii in range(21):
    #
    prior_temp =GFAS_total_Flux_per_area[ii,:,:] * area
    prior_temp[np.where(Forest_mask==0)] = np.nan
    prior_GFAS_MaySep_vec[ii] = np.nansum(prior_temp)
    #
# ----------------------------------------------------------------------------------------
...
plt.savefig('Figures/Byrne_etal_FigS1.png', dpi=300)
...
plt.savefig('Figures/Byrne_etal_Fig1.png', dpi=300)
